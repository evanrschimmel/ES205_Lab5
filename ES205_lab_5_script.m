...
h1_0_single = h1data_single(1); % initial water height, in
sim('ES205_lab_5_upper_model_orig')
see_upper_orig = sqrt(sum((h1_single_orig-h1data_single).^2)./(length(h1data_single)-2));

tf_lower = tdata_lower(end);
h2_0_single = h2data_single(1); % initial water height, in
sim('ES205_lab_5_lower_model_orig')
see_lower_orig = sqrt(sum((h2_single_orig-h2data_single).^2)./(length(h2data_single)-2));

tf_double = tdata_double(end);
h1_0_double = h1data_double(1); % initial water height, in
h2_0_double = h2data_double(2); % initial water height, in
...
Cd1_mod=values1(si);
sim('ES205_lab_5_upper_model_mod')
see_upper_mod = sqrt(sum((h1_single_mod-h1data_single).^2)./(length(h1data_single)-2));
...
Cd2_mod = values2(si);
sim('ES205_lab_5_lower_model_mod')
see_lower_mod = sqrt(sum((h2_single_mod-h2data_single).^2)./(length(h2data_single)-2));
...
figure
plot(tdata_upper,h1data_single,'b-',t_upper,h1_single_orig,'b:',t_upper,h1_single_mod,'b--')
xlabel('Time [s]')
...
figure
plot(tdata_lower,h2data_single,'r-',t_lower,h2_single_orig,'r:',t_lower,h2_single_mod,'r--')
xlabel('Time [s]')
ylabel('Water height [in]')
...
figure
plot(tdata_double,h1data_double,'b-',tdata_double,h2data_double,'r-',t_double,h1_double_orig,'b:',t_double,h1_double_mod,'b--',t_double,h2_double_orig,'r:',t_double,h2_double_mod,'r--')
xlabel('Time [s]')
ylabel('Water height [in]')
